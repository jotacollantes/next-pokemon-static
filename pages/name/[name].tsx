...
//* Hacemos la llamada Api en tiempo de build
  ...
  const newArrayPokemonsByName=data.results.map((pokemon)=>(pokemon.name)) 
   return {
    // paths:[
    //     { params: { name: 'bulbasur' } },
    //  ],
    //!Genero dinamicante los paths
    paths: newArrayPokemonsByName.map((namePokemon) => {
      return { params: { name: namePokemon } };
    }),
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const { name } = params as { name: string }; //* tipado
   return {
    props: {
      pokemon: await fetchPokemons(name),
    },
  };
};
